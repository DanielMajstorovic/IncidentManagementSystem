...
@Component({
  ...
  styleUrls: ['./admin-incidents.component.scss'],
})
export class AdminIncidentsComponent implements OnInit, OnDestroy {
  ...
  // Enhanced modal opening for admin
  openIncidentModal(incident: Incident): void {
    // Reset modal state including translation properties
    this.updateModalState({
      isOpen: true,
      ...
      translation: null,
      isTranslated: false,
      isTranslating: true, // Start translation loading immediately
    });

    this.incidentModal?.show();

    // Load images (same logic as moderator)
    ...
    this.translationService
      ...
      .pipe(finalize(() => this.updateModalState({ isTranslating: false })))
  ...
  private loadIncidents(): void {
    ...
    // Admin can see all statuses by default (no default status filter)
    ...
    this.incidentService.filterIncidents(request).subscribe({
      next: (response) => {
        ...
        // Update counters for admin dashboard
        this.totalIncidents = response.content.length;
        this.reportedCount = response.content.filter(i => i.status === 'REPORTED').length;
        this.approvedCount = response.content.filter(i => i.status === 'APPROVED').length;
        this.rejectedCount = response.content.filter(i => i.status === 'REJECTED').length;
        ...
        response.content.forEach((incident) => {
          ...
          // Use standard colors for all incidents
          const color =
            iconColorMap[incident.type] ||
  ...
  private initializeModal(): void {
    const modalElement = document.getElementById('adminIncidentModal');
    if (modalElement) {
